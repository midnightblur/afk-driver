...
- **Your fragment declares its partition** (`partition.id`, `partition.classes`, `partition.seed`) and omits the classes outside it. Copy the `run` block the caller handed you into the fragment verbatim — the fold refuses a fragment that answers no stated question. Write every search you record in the command grammar (`LEDGER-FORMAT.md` § "Command families"; build it with `contract.build_command` where you run Python) — a search needing two conditions is two searches, and a command outside the grammar refuses the fold. Merge keys and the conflict rule are the caller's; write the ids the format defines and nothing else.

...
- **Truth-grounding bar.** A claim the caller will act on — above all "X does not exist / never happens / is impossible" — meets `tools/payable/ai-agents/harness/shared/core-services.md` § "Truth grounding": traced to code across every boundary; anything unchecked returned as `unverified: <reason>`, never as fact.

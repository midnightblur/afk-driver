...
- **Triage, don't relay.** For each command/suite: verdict (`green`/`red`/`blocked`), counts (passed/failed/skipped), and for each failure a one-line digest — failing item, error class, `file:line` where the output points. No raw stack traces or log excerpts beyond one decisive line each.

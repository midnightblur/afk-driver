...
Apply the visualization rules in [VISUALIZATION.md](VISUALIZATION.md) — the Toolkit (Mermaid diagram-type table) and the diagram-quality rules — when producing every non-trivial layer's visual.

## SDD template

Write SDD.md using the template in [SDD-TEMPLATE.md](SDD-TEMPLATE.md). Each section's `Required visuals:` annotation in that template is binding.

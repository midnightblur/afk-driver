...
- cwd is a clean worktree on the parent branch (`kapteyn/development/{username}/{enh_id_lower}`). Create worktree + branch yourself if it doesn't exist yet.
...
1. **Read the contract.** `ctx_read` `plan/{NNNN-slug}.md`; parse its sections against the subtask contract (`## Goal / Complexity / Design refs / Scope / Seams / Acceptance / Produces / Consumes / Verification / Parent PRD / Parent SDD / Blocked by / Conflict procedure`; a missing `## Complexity` reads as `standard` — the field sizes the *spawning* orchestrator's dispatch, not this skill's behaviour). Read `plan/PLAN.md` for rank order, the `## Blocked by` graph, the seam register. Read the `## Parent PRD` file. When the read set goes beyond the contract file itself (PLAN.md, the parent PRD, cited SDD sections/ADRs), an `afk-reader` subagent builds the citation/context digest and you read only the contract + its digest, per `DELEGATION.md` (plugin root).
...
10. **Status → `reviewing`; independent review gate.** Every tier is green (Step 8) and the `## Produces` anchors confirmed (Step 9) — but green tiers don't prove the code honours the CLAUDE.md rules, covers the whole spec, or is free of risky refactoring. Flip the tracker cell to `reviewing` and run **`/afk:review {NNNN-slug}`** before declaring done. It spawns fresh, independent subagents (they never see your reasoning) across the seven concerns and is **read-only** — it returns one verdict line: `REVIEW: <verdict> — crit=… high=… med=… low=… [findings: <path>]` (grammar owned by `/afk:review` — lockstep copy here because this step parses it).
    ...
    - **`advisory`** (only `medium`/`low`) → don't block on nits. The findings already live in `plan/review/*.md`; add a brief MR note, then proceed to Step 11.
...
11. **Update the tracker.** Set the subtask's PLAN.md row `Status` to `done`; stamp the `Last updated` date. The subtask file itself round-trips verbatim — this skill writes no per-run note anywhere; the run's signals live in the PLAN.md `Status`/`blocked(…)` cell, `plan/JOURNAL.md`, and `plan/review/*.md`.

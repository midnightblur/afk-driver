...
4. **Plan inside Scope.** Stay strictly within the `## Scope` globs. Check your diff against those globs and the forbidden-pattern list before committing — entity classes fine; hand-written `UpgradeGroup_*.java` / liquibase changesets / `db/changelog/*` edits are not (see Hard rules).
   ...
   **Close the ground the slice stands on, before any code.** Cited mode: re-run the seed map for every symbol this slice's `## Seams` names and diff its `hit_ids` against the ledger the seam's investigation left — recipe in [CITED-MODE.md](CITED-MODE.md) § "Ground diff". No new hits → proceed. New hits → one delta `afk-tracer` over the delta alone, folded back with `merge_fragments.py`. Uncited mode: one `/afk:investigate` Q3 run per pre-existing symbol the slice changes. A `partial` verdict on a load-bearing claim is a decision the run cannot take: park it as `needs_decision` (Step 13) carrying the ledger path.

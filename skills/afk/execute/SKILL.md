...
4. **Plan inside Scope.** Stay strictly within the `## Scope` globs. Check your diff against those globs and the forbidden-pattern list before committing — entity classes fine; hand-written `UpgradeGroup_*.java` / liquibase changesets / `db/changelog/*` edits are not (see Hard rules).
   ...
   **Close the ground the slice stands on, before any code.** Cited mode: re-take the ground for every symbol this slice's `## Seams` names and compare it with the ledger the seam's investigation left — recipe in [CITED-MODE.md](CITED-MODE.md) § "Ground diff". The ground held → proceed. It moved → one fresh `/afk:investigate` run at the current head, whose id this slice's journal line and `OUTCOME:` pointer name. Uncited mode: one `/afk:investigate` Q3 run per pre-existing symbol the slice changes. A `partial` verdict on a load-bearing claim is a decision the run cannot take: park it as `needs_decision` (Step 13) carrying the ledger path.
...
10. **Status → `reviewing`; review settle loop.** Every tier green (Step 8) and `## Produces` anchors confirmed (Step 9): `plan-status.sh {plan-dir} {NNNN-slug} reviewing`, then gate through the settle loop (`skills/afk/review/SETTLEMENT.md` — round structure, fix-or-dispute, dispute adjudication, referee-kept termination; **you are its referee**). Per round, run **`/afk:review {NNNN-slug} --tag r{n}`** — it spawns fresh, independent subagents (they never see your reasoning) across its concern roster (per the plan's gate policy on the first round; delta rounds consolidate to its delta roster — that skill owns the scaling) and is **read-only** — returning one verdict line: `REVIEW: <verdict> — crit=… high=… med=… low=… [findings: <path>]` (grammar owned by `/afk:review` — lockstep copy here because this step parses it). The loop, not any single verdict, decides the gate: every actionable finding — `medium`/`low` included — is fixed or disputed per SETTLEMENT.md until a round comes back with nothing actionable.
    ...
    - **Deferral rule (lean policy only).** When the resolved gate policy is `lean` (`/afk:review` "Gate policy"), declare SETTLEMENT.md's deferral rule for this gate: **`medium`/`low` findings of class `smell` / `compliance` / `design` route to the feature-level review gate** — recorded `deferred` in the round's outcomes file, never settled here (they surface as open advisories in `plan/review/INDEX.md`; the feature gate sweeps them). `critical`/`high` of any class, and every finding of the other classes, settle in this loop as below. Under `full`, nothing defers.
    - **Fix routing by `class`** (SETTLEMENT.md owns the loop; this table owns the routing):
      ...
      - `product-debt` → never gates, but it does owe a home: land its `## Known debt` entry in the nearest `AGENTS.md` via `/afk:agents-md`, and record that path in this round's `*.outcomes.json` (`/afk:review` "Product-debt homes"). `/afk:preflight` PF-4d refuses to go green on one with no home.
...
11. **Reconcile the glossary, then update the tracker.** If implementation revealed a domain term's actual semantics differ from its `GLOSSARY.md` definition (owning glossary located via `GLOSSARY-MAP.md`; format per `/afk:glossary`), update the entry and commit it separately: `[{NNNN-slug}] glossary: <terms>`. Then `plan-status.sh {plan-dir} {NNNN-slug} done`. The subtask file round-trips verbatim — this skill writes no per-run note anywhere; the run's signals live in the PLAN.md `Status`/`blocked(…)` cell, `plan/JOURNAL.md`, `plan/DECISIONS.md`, and `plan/review/*.md`.
...
## Hard rules (inherited from the repository's own AGENTS.md)
...
- **Plugin issues go upstream.** A failure whose cause is the plugin itself → route it per `skills/afk/lessons/CAPTURE.md` "A plugin defect is an issue"; the outcome report carries its `ISSUE:` line.

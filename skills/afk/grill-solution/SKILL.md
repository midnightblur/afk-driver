...
1. **State the layer and what it covers in one sentence, then close its premises.** Before the layer's first question, spawn one `/afk:investigate` run per premise the layer's questions or your draft recommendations rest on — one background set, in one message, `--design-phase` (`DELEGATION.md`). A premise the run answers `frontier(B14)` is answered: the code that would settle it is outside this repository, and the layer records that. A layer locks only once every one of its premise ledgers has returned. A premise — inherited or returned — locks on a `closed`/`closed-with-frontier` INV id written on the row, or on a gap the human acknowledges there in words; anything else keeps the layer open.
...
3. **Triage the layer's concerns per `skills/afk/grill-requirements/TRIAGE.md`, then present them in the layer's round per `../grill-requirements/ROUND.md`** (an override returns as debate next round). Recommend an answer per item. Force ≥2 alternatives — and for **L1–L3** decisions, present one deliberately different **third option** before settling: a different paradigm (event-driven vs sync, buy vs build, denormalize vs join), not a variant of the front-runner, with an honest cost. Two look-alike options is how the obvious answer wins unexamined. Capture each item's rationale before the round is presented. **When the question's premise OR the user's answer references existing infrastructure, apply [GROUNDING-RULE.md](GROUNDING-RULE.md) before locking in.** Lavish is this session's default surface — **session-default** per LAVISH.md (RP-1, playbook `comparison`), **kit path** — author the round JSON and run the render script per `LAVISH-KIT.md`, never HTML: render each round, debate alternatives laid side by side, into one phase artifact re-rendered at each round boundary — mandatory per LAVISH.md's Primary-path rule; a licensed skip (driven mode / render failure / user opt-out) per that file falls back to the prose recap above.
4. **Before descending, restate the locked decisions** so the user can challenge. Each layer's row carries the investigation ids behind its premises. As each layer locks, checkpoint it (one row) into this skill's section of the ticket folder's `GRILL-LOG.md` per `skills/afk/grill-requirements/GRILL-LOG-FORMAT.md` — a 9-layer grill is long; the log lets a paused or compacted session resume at the right layer with the locked set intact, and records L9 seam verdicts until the SDD lands.
...
- **Every premise carries a closed ledger** — verdict `closed` or `closed-with-frontier` — **or its `unverified` and `frontier` items are listed and acknowledged by the human** (per the Grounding rule). A design built on a fictional premise isn't exhausted, it's poisoned.

...
Interview the user relentlessly about every aspect of the architecture until reaching a shared understanding. Walk the design tree **top-down across 8 layers**. Resolve each layer before descending — choices at a lower layer are brittle when choices at a higher layer haven't been pinned (e.g. picking Strategy at L8 before deciding at L4 whether rendering is sync or async means the strategy interface might need to return a `Future<T>` you didn't plan for).

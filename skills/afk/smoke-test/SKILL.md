...
Those suites live on permanently in the repository (the `e2e/browser` catalog + the `api` scenarios), so CI, scheduled jobs, and ad-hoc human sanity runs invoke them directly. This skill is the **AFK-facing gate** plus a **manual runner** — not the only way the suites run.
...
- `target` *(optional)* — the running app both suites hit (browser navigates it; API client calls its REST surface): `local` (default — dev server), `staging`, or an explicit base URL. A base URL from a feature environment counts as explicit: run the repository's `verification.env` status command and read the base URL out of its answer. Never silently default to a target the human didn't pick when release-gating; `local` for dev sanity, `staging` for acceptance.
...
3. **Precondition — app reachable + suite env set.** Confirm the `target` app is up (hit its base/health URL). Not reachable → `env_unreachable`; tell the human to start the app / bring up the env — or provision one yourself with the repository's `verification.env` up command, then re-run with that environment's base URL as `target`. Also confirm **each modality's** run env — the browser suite's auth token / base URL per `the repository's `e2e/browser` suite/README.md`, and the API suite's token / base URL per the `api` suite's own authoring recipe (API client mints via `../core`). This gate requires a **real running app**.
...
- **Owns only the gate's surface in `PLAN.md`** — the gate section's table `Status` cells (full or minimal shape), its `Last run` line, its append-only `Run history` list, and the header `Feature:` line. Everything else in `PLAN.md` round-trips verbatim (the `## Progress tracker` status column stays `/afk-toolkit:execute`'s). Outside `PLAN.md` it appends to `plan/JOURNAL.md` (append-only) and upserts the `Smoke gate` row of the ticket `INDEX.md` — nothing else.
...
- **Authors no specs.** Spec code is the terminal `NNNN-smoke-e2e` / `NNNN-smoke-api` subtasks' reviewed work, built per the canonical recipes the `e2e/browser` suite's own authoring recipe and the `api` suite's own authoring recipe. A scenario needing a new/changed spec is a subtask edit (re-run `/afk-toolkit:execute` on it) — or a scenario redesign (`/afk-toolkit:grill-verification`) — not an edit from inside the gate. A suite missing or red because scenarios were authored ad hoc → point the fix back at the relevant `AUTHORING.md` — the standard the build subtask owes.
...
Feature green → complete; the feature is now demoable, so **`/afk-toolkit:to-demo-plan`** can synthesize the ≤1h `DEMO-PLAN.md` for showing it to product owners + QA (its journeys are the ones just run). Subtask MRs are still **Draft** and the human's to review + merge (`/afk-toolkit:execute` left them that way on purpose). After the merges, the smoke suites stay under the repository's verification suites (`ui-e2e` + `api`) and are picked up by CI / scheduled verification / manual sanity runs from then on — re-run this skill (or the raw suites) whenever you want to re-confirm the system is sane.

Feature red → fix the failing scenario at its source (re-run `/afk-toolkit:execute` on the owning subtask, `/afk-toolkit:grill-verification` if the scenario itself was wrong, or `/afk-toolkit:grill-solution` if a binding decision is wrong), then re-run `/afk-toolkit:smoke-test`.

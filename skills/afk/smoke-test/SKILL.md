---
name: smoke-test
description: The feature-level completion gate. After every subtask in a local plan is done, run the feature's already-implemented verification suites — the browser UI journeys (ui-e2e) and the direct-REST API contracts (api), both under 11700-payable/verification — against a running app and, only on green across both, stamp the feature complete in PLAN.md. This skill only EXECUTES scenarios that were designed by `/afk:grill-verification` and built by the terminal smoke-e2e / smoke-api subtasks — it authors nothing. Distinct from the per-subtask api / e2e/browser tiers: this verifies the integrated whole. Present only when a `## Feature smoke gate` exists in PLAN.md (i.e. a VERIFICATION-PLAN.md drove a build subtask). Also the human's standalone/manual runner; CI and scheduled jobs run the same suites directly. Touches no Jira and merges nothing. Use as the final acceptance check once a feature's subtasks have all landed, or to manually re-verify a feature's sanity.
---

...
Feature green → complete; the feature is now demoable, so **`/afk:to-meeting-d`** can synthesize the ≤1h `DEMO-PLAN.md` for showing it to product owners + QA (its journeys are the ones just run). Subtask MRs are still **Draft** and the human's to review + merge (`/afk:execute` left them that way on purpose). After the merges, the smoke suites stay under the repository's verification suites (`ui-e2e` + `api`) and are picked up by CI / scheduled verification / manual sanity runs from then on — re-run this skill (or the raw suites) whenever you want to re-confirm the system is sane.

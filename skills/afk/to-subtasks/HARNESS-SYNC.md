...
updates the AGENTS.md harness so the next agent discovers the shipped feature,
...
Delegates the write to `/afk:agents-md`; the subtask only scopes that to this
...
judgment calls surfaced to the human via `/afk:agents-md`'s propose→approve gate — no
...
```
## Goal
Sync the AGENTS.md harness for {Feature} so the next agent discovers it and knows how to
use/extend it, settle the staples registry, and emit the trace matrix. Run /afk:agents-md scoped
to THIS feature's net diff (this branch vs the parent), capturing: (1) ONE lazily-loaded
instruction in the nearest component/leaf AGENTS.md — max 3 dense sentences on how to add/extend
or invoke the feature, pointing at the key types + the spec dir; (2) ONE awareness sentence in the
affected service-root AGENTS.md that names the feature and leads to (1); and (3) ONLY IF
...
## Scope
- {service}/**/AGENTS.md        # the affected service's AGENTS.md chain (nearest leaf + service root)
- {service}/STAPLES.md          # only if this feature mints a new staple or advances an existing one's Reference
- {other services}/**/AGENTS.md # only the other service dirs the feature genuinely spans
- {tooling dir}/**/AGENTS.md    # only when the feature is tooling itself
- {plan dir}/TRACE.md           # the trace matrix this subtask emits
# docs only — NO source edits

## Acceptance
- [ ] The how-to note lives in the nearest component/leaf AGENTS.md, not the service root
- [ ] The service-root AGENTS.md carries exactly ONE awareness sentence leading to that note
...
- [ ] Nothing added restates code; every line clears the /afk:agents-md inclusion bar
- [ ] Written via /afk:agents-md
...
|------|---------------------------|--------|
| static | grep the awareness sentence's path in the service-root AGENTS.md; confirm that leaf AGENTS.md exists on disk | the pointer resolves |

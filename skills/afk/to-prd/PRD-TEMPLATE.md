...
- **Data scope is by company and/or vendor, never tenant** (where the repository's own `AGENTS.md` states a tenancy model, follow it). Record *which entity* is scoped, not concrete values (FOS-configured at runtime).

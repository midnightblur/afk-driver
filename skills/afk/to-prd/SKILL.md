...
Write `PRD.md` (+ any requirement ADRs) to the repo and stop. Publishing to a tracker = separate **`/afk-toolkit:to-ticket`**, run afterwards.
...
- **`/afk-toolkit:to-ticket`** — distill the PRD to a requirements-level ticket description and publish it into the **existing** parent ticket as native Jira formatting (mermaid rendered + embedded); idempotent, preserves product-owner content. Requires a parent key — doesn't create the ticket.
- **`/afk-toolkit:prototype`** *(optional)* — if the feature has meaningful net-new UI, craft the screens now against the **real frontend's** look, before the SDD locks decisions. Writes `PROTOTYPE.md` + chosen HTML sibling to the PRD; self-gates `no_ui` for backend-only. Feeds `/afk-toolkit:grill-solution` and gives `/afk-toolkit:grill-verification`'s UI journeys a concrete screen.
- **`/afk-toolkit:grill-solution`** — interview the architecture top-down L1 → L9.
- **`/afk-toolkit:to-sdd`** — synthesize SDD + design ADRs. Without an SDD the plan slices uncited (PRD-only).
- **`/afk-toolkit:grill-verification`** *(optional)* → **`/afk-toolkit:to-verification-plan`** — design verification scenarios; the grill interviews, then `/afk-toolkit:to-verification-plan` writes `VERIFICATION-PLAN.md`. Post-PRD it can design the **UI journeys** (User Stories are usually concrete enough; the walk often surfaces PRD gaps) and **defers API scenarios** until an SDD settles the endpoints — re-run both after `/afk-toolkit:to-sdd`. Its plan makes `/afk-toolkit:to-subtasks` add the feature smoke-test gate.

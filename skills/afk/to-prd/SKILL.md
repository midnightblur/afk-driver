...
3. Write the PRD using the format in [PRD-TEMPLATE.md](./PRD-TEMPLATE.md). Apply its **concision doctrine** (the banner at the top of the template) throughout: capture the design precisely but state each fact in exactly one home and reference it by ID elsewhere. On any feature with a set of parallel things (rules, states, roles, endpoints), define them once in a catalog table and have User Stories / Acceptance Criteria / Implementation Decisions reference rows rather than re-enumerate them — this is what keeps a complex PRD short enough to stay readable and to publish into Jira without hitting the description length limit.

...
- Send the end-of-run notification and report. Read `plan/DECISIONS.md` first: every entry whose `decision(D-{n})` journal line falls inside this run's window goes on the report's `decisions:` line (protocol: `DECISIONS.md`, plugin root) — the human audits hands-off decisions at run end, never mid-run. Driven subtask runs draft workflow lessons nobody has approved yet (`skills/afk/lessons/CAPTURE.md`); read the open count via `bash $AFK_PLUGIN_ROOT/hooks/lesson-digest.sh --count` from the worktree root — a non-zero count tells the human `/afk:lessons apply` is worth a stop.
...
issues: <n> filed, <m> queued — <url or queued path>, … (every `ISSUE:` line from subtask outcomes and this run; omit the line when 0)

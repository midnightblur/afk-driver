...
When the user states how something works — existing behaviour, a constraint, an ownership boundary — verify against the code before building requirements on it, per [../grill-solution/GROUNDING-RULE.md](../grill-solution/GROUNDING-RULE.md) (trigger phrases, verify-by-claim-type, miss-handling); delegate the verification per `DELEGATION.md`. Surface a contradiction in the next round: "Your code cancels entire Orders, but you just said partial cancellation is possible — which is right?"

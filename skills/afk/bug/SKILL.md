...
3. **Publish if config allows (S1 → S2).** Resolve `trackerAssignee` (K1) — `afk-config.py resolve trackerAssignee`, the developer's own value (a person has no committed default); nothing resolves it → fail closed per [CONFIG.md](CONFIG.md): the bug stays `captured` (S1) (AC-001). Present → spawn the **Publisher** subagent to run `scripts/publish_bug.py create` (assignee K1, evidence bundle, screenshots, optional FixVersion) then `transition` to Dev-Pending. Record the returned key: rename the bug directory to the Jira key and write the `published` (S2) transition. A surfaced Publisher failure leaves the bug `captured` with the error in `history` — the capture is intact; retry via `status`.

...
5. **Create the fixer worktree (S4) (AC-009).** Resolve `worktreeBasePath` (K3) — normally derived, so this rarely fails; nothing resolves it → fail closed per [CONFIG.md](CONFIG.md): dispatch refused, the bug stays `published` (S2). Run `scripts/create-worktree` for a fix branch off the base, passing `--no-open` so the **IDE is not launched** (a human running the same script still gets the IDE by default). Parse the script's last line: `WORKTREE_PATH=<abs>` on success, `ERROR=<reason>` on failure (contract in `scripts/create-worktree`). Record `worktreePath` + `baseBranch` and write the `fixing` (S4) transition.

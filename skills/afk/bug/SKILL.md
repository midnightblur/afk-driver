...
3. **Publish if config allows (S1 → S2).** Resolve `trackerAssignee` (K1) — `afk-config.py resolve trackerAssignee`, which covers the developer's own value and the repository's committed default; nothing resolves it → fail closed per [CONFIG.md](CONFIG.md): the bug stays `captured` (S1) (AC-001). Present → spawn the **Publisher** subagent to run `scripts/publish_bug.py create` (assignee K1, evidence bundle, screenshots, optional FixVersion) then `transition` to Dev-Pending. Record the returned key: rename the bug directory to the Jira key and write the `published` (S2) transition. A surfaced Publisher failure leaves the bug `captured` with the error in `history` — the capture is intact; retry via `status`.
...
5. **Create the fixer worktree (S4) (AC-009).** Resolve `worktreeBasePath` (K3) — normally derived, so this rarely fails; nothing resolves it → fail closed per [CONFIG.md](CONFIG.md): dispatch refused, the bug stays `published` (S2). Run the plugin's `scripts/create-worktree` for a fix branch off the base, passing `--no-open` so the **IDE is not launched** (a human running the same script still gets the IDE by default). It provisions the worktree for whichever build gates the repository selected. Parse the script's last line: `WORKTREE_PATH=<abs>` on success, `ERROR=<reason>` on failure (contract in `scripts/create-worktree`). Record `worktreePath` + `baseBranch` and write the `fixing` (S4) transition.
6. **Spawn the fixer with its full input set.** Resolve `mrReviewer` (K2) and `mrAssignee` (K5) — the developer's own values, and nothing else: no committed file names a person. Both come from the one resolver, `afk-config.py resolve <key>`. Spawn an **`afk-implementor`** subagent (the fixer writes product code — implementation tier, `DELEGATION.md` "Model selection") with [FIXER-PROMPT.md](FIXER-PROMPT.md), filling **all** of the placeholders it declares: `{BUNDLE_PATH}` (the bug's `bundle.md`), `{WORKTREE_PATH}` + `{FIX_BRANCH}` (from step 5), `{BASE_BRANCH}` (the recorded base), `{MR_REVIEWER}` (K2), and `{MR_ASSIGNEE}` (K5). K2 absent, or the literal `none` → fail closed per [CONFIG.md](CONFIG.md): pass an empty reviewer — the fix tops out at `fix-pushed` (S6), never `mr-ready` (S7); dispatch still proceeds. K5 absent → pass an empty assignee; the change opens with no assignee and dispatch is unaffected. The fixer reproduces on the clean base first, fixes, tests, pushes, opens a Draft MR, babysits CI, and flips Ready — all inside its own worktree.

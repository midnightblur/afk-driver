...
- `{MR_ASSIGNEE}` — forge username the Draft change is assigned to at creation (K5). May be empty (no assignee configured); empty opens the change with no assignee and gates nothing.
...
5. **Babysit CI, then flip Ready.** Wait for the pipeline using the forge's `ci-wait` verb — `afk_adapter forge ci-wait '{"id":"<change-ref>","budget":10800,"interval":120}'` (budget 10800 s, poll every 120 s). Route on its exit code:
   - **exit 0** (pipeline green) → **first check `{MR_REVIEWER}`**: if it is empty, never flip Ready — leave the MR Draft and return `BUGFIX: fix-pushed — pushed; no MR reviewer configured (K2), MR left Draft`. Otherwise flip the change Ready and assign the reviewer: `afk_adapter forge change-ready '{"id":"<change-ref>"}'` then `afk_adapter forge change-reviewers '{"id":"<change-ref>","reviewers":"{MR_REVIEWER}"}'`. The change was already assigned to `{MR_ASSIGNEE}` at creation (step 4); this reviewer drives the MR, and the human owns the merge. Return `BUGFIX: mr-ready — <summary>`.

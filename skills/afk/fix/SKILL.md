...
Where a coverage ledger covers this bug's path, read its B13 rows for the statements the fix now contradicts (class semantics: `${AFK_PLUGIN_ROOT}/INVESTIGATION.md`), and route each by the table above.

...
Do NOT interview. Brief section can't be filled from source docs → STOP, tell user to run `/afk-toolkit:grill-solution` and re-emit the SDD; don't invent.

## Process

1. **Locate the source docs** in the ticket spec folder (path convention: `skills/afk/to-prd/SKILL.md`, "Monorepo conventions") — sibling layout: `PRD.md`, `SDD.md`, `adr/requirements/NNNN-*.md` (requirement-level, from `/afk-toolkit:to-prd`), `adr/design/NNNN-*.md` (design-level, from `/afk-toolkit:to-sdd`). Brief lands at `.../{TICKET-ID}/DESIGN-BRIEF.md` (sibling).
...
2. **Refuse if the SDD is incomplete.** SDD §13 Open Questions non-empty with `Blocks executor? = yes` rows → do NOT emit a brief; design not stable enough to summarize. Tell the user to resolve via `/afk-toolkit:grill-solution` first.
...
- **Strict synthesis.** Every claim traces back to PRD / SDD / an ADR. No new decisions, alternatives, or rationale. Section can't be filled from sources → refuse; bounce to `/afk-toolkit:grill-solution` + `/afk-toolkit:to-sdd`.
...
The brief is for stakeholder review, not the executor — the binding contract for AFK subtasks is the SDD + ADRs, not the brief. After the brief is written and shared out of band, run **`/afk-toolkit:to-subtasks`** to slice PRD + SDD + ADRs into a local execution plan. The brief is not in the executor's reading list.

...
- **mr:** validate the forge (`afk_adapter forge auth-status`; `authenticated: false` or an `unavailable` answer → `failed(forge_unavailable)`, showing the reason it gives). Fetch with `afk_adapter forge change-fetch '{"id":"{change-ref-or-url}","out_dir":"{scratch}"}'` → `mr.json` + `mr.diff` in the provider scratch directory (`CAPABILITIES.md`), plus the normalized change object on stdout. **Spec discovery** (best-effort, enriches SEC-1): cascade against the MR description — ticket URL regex → markdown-link key → free-text key → commit messages; a key whose repo has `specs/**/{KEY}/PRD.md` → read it; else Jira MCP / WebFetch; all misses → proceed, SEC-1 falls back to the MR description + commits. Draft/closed/merged MRs all proceed.

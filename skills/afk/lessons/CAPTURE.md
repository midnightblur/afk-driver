...
as `/afk:agents-md`'s inclusion bar). When unsure, don't capture.
...
| Condition | Do |
|---|---|
| Human present **and** target is AGENTS.md / role sidecars / `.claude/rules` / `STAPLES.md` | Delegate to `/afk:agents-md` (its propose → approve → write). On approved write: append `opened` then `applied`. Declined: append `opened` then `rejected`. |
...
**Self-contained** = one file, no `AGENTS.md` "Lockstep" partner, no row in

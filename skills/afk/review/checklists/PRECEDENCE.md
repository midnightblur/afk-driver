...
Rules binding every checklist item marked as **baseline** (book-derived catalog items, cited `(Fowler)`, `(APoSD)`, `(EJ-nn)`, `(CC2)`, `(DDD)`, `(Nygard)`, `(Newman)`, `(PoEAA)`, `(Meszaros)`, `(Bloch-API)`, `(Beck)`). Documented-rule checks (AGENTS.md chain, spec, scope) are NOT baseline — these rules don't soften them.

1. **The repo overrides — but report the conflict.** A documented standard in the target repo's AGENTS.md chain / `STAPLES.md` / ADRs wins where it endorses what a baseline item would flag. Don't suppress silently: emit the finding as `class: pattern-debt`, severity `low`, quoting BOTH the baseline item and the overriding repo rule. `pattern-debt` never blocks; it feeds the debt ledger. Net-new modules with no established local pattern get the baseline at full strength — no override applies.
...
- **Open the document, not its title.** Before overturning a cited rule, or asserting none exists, read the cited file and the `AGENTS.md` that auto-loads for the directory under review. An accepted amendment can add a rule the document's title never mentions.

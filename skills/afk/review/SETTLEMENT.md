...
Trigger: **two consecutive rounds file a finding of that shape.** From the next round the loop is no longer delta-scoped. The referee passes the review pass `--scope-escalated`, and each reviewer reads the whole surface the feature touches: the component's code and its `AGENTS.md`, the specs and ADRs stating the same facts, the review records the loop has written, and the cross-service consumers of anything the feature publishes.

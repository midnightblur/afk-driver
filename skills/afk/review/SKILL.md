...
One home per concern: `checklists/{concern}.md`, pasted into the reviewer's prompt (paste rule above) together with `checklists/PRECEDENCE.md` (baseline precedence, the `pattern-debt` rule, one owner per smell, the mandatory open question). Each file states its default `class`, its escalations, and its "Not yours" exclusions. Design-level files also carry a `## Guardrails` digest for design-time consumers. The `test-veracity` file owns the sampled mutation-probe rule (gate mode only). `checklists/delta-sweep.md` (delta-round consolidation) and `checklists/consistency-sweep.md` (scope escalation) are roster-entry checklists — same mechanics, neither one of the 11 concerns.

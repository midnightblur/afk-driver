...
How every AFK skill reports to the human. Binding on any skill that emits a status line, a push notification, or a run report. The goal: a human who knows the *domain* but not the *workflow vocabulary* can follow every report without opening another file.
...
- **Abbreviations expand at first use** per report or artifact — "Product Requirements Document (PRD)" once, then abbreviate freely. Applies to domain abbreviations too.

...
- **Reviews can now classify a finding `product-debt`**: a real shortcoming in
  ...
  rejected instead of proposing it again. `/afk:preflight` gains **PF-4d**,
...
- **`create-worktree` no longer clones `.claude/TODO.md`.** The `/afk:todo` list is per-worktree state, not shared config — a fresh worktree now starts with an empty todo list instead of a stale copy of the source checkout's.
...
- **`/afk:setup` now installs `openpyxl`.** `/afk:review-qa-tests` imports it to annotate QA's `.xlsx` sheet, but it was registered nowhere — so a fresh machine could pass the doctor green and still die at first use (`MANIFEST.md` P4). The same audit sweep registered two env toggles that were live but undocumented: `AFK_DRIVEN` (the hands-off marker `/afk:gc` refuses on) and `CLAUDE_PROJECT_DIR` (harness-set, locates the adopted harness gates). Re-run `/afk:setup`.

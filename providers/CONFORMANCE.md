...
| Stop gates block after a plugin edit | pass 2026-09-01 | FAIL 2026-09-02 (round 3) | Second harness: the launcher reaches `stop-gates.sh` and the gate produces its verdict, but stderr plus exit 2 is recorded as a failed hook, not a block — the session finishes anyway. Its decision contract is proven (a `permissionDecision` deny object blocked a PreToolUse in round 2). Fixed here: a block is emitted as a `{"decision":"block","reason":…}` object on stdout as well as on stderr, with the exit code named per adapter; awaiting re-probe round 4 |
...
| Stop block decision object is honoured | pass 2026-09-02 | awaiting round 4 | First harness, live rig: with the adapter exit code set to 0 so only the decision object could carry the verdict, an unregistered scratch skill produced a real Stop block carrying the gate findings. The stderr-plus-exit-2 form is therefore not required here, and one emission serves both harnesses |

## Unresolved items

- Round 3 (2026-09-02) cleared the shell-resolution and credential causes: handlers, the guard denial and the Jira server are all native-green on the second harness. One probe remains: a Stop block travelled only as stderr plus exit 2, which that harness records as a failed hook rather than a verdict. Emitting the block as a decision object as well is fixed here and awaits re-probe round 4. Until it comes back green, the Stop gates are advisory on that harness — they run and report, but do not stop a turn.

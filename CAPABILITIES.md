...
- Blocking: PreToolUse deny envelope; Stop stderr plus exit 2.
- Commands keep `${CLAUDE_PLUGIN_ROOT}`. Repository hooks use `${CLAUDE_PROJECT_DIR:-$(git -C "$PWD" rev-parse --show-toplevel)}`.

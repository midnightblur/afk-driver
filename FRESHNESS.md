...
| `skills/utils/investigate/LEDGER-FORMAT.md` | `/afk:investigate` | a ledger table, a field, a status set, the verdict rule, the merge rules, the stable-key rule, the fragment grammar, the file names, the location rule, or the report/reply shape changes — move the whole set in the same commit: the writers (the emitter `skills/utils/investigate/scripts/seed_map.py`, the fragment writer `agents/afk-tracer.md`, the merging skill `skills/utils/investigate/SKILL.md`), the parser `skills/utils/investigate/scripts/validate_coverage.py`, the shared contract module `skills/utils/investigate/scripts/contract.py`, the six test files (`scripts/tests/test_seed_map.py`, `test_seed_map_reach.py`, `test_seed_map_binding.py`, `test_validate_coverage.py`, `test_validate_coverage_shapes.py`, `test_validate_coverage_binding.py`), and every consumer of a status or verdict value it names |
| `skills/utils/investigate/scripts/` (`seed_map.py`, `validate_coverage.py`) | `/afk:investigate` | the seed-map JSON shape, a default boundary pattern, a name-form rule, a status rule, or a validation rule changes — `skills/utils/investigate/LEDGER-FORMAT.md` and all four `scripts/tests/test_seed_map*.py` / `test_validate_coverage*.py` files move in the same commit |

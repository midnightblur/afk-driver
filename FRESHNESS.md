...
| `skills/utils/investigate/LEDGER-FORMAT.md` | `/afk:investigate` | a ledger table, a field, a status set, the file names, the location rule, or the report/reply shape changes — `scripts/validate_coverage.py` (the parser) and `scripts/tests/validate_coverage_smoke.py` move in the same commit |
| `skills/utils/investigate/scripts/` (`seed_map.py`, `validate_coverage.py` + smoke tests) | `/afk:investigate` | the seed-map JSON shape, a default boundary pattern, or a validation rule changes — `LEDGER-FORMAT.md`, `skills/utils/investigate/scripts/tests/seed_map_smoke.py` and `skills/utils/investigate/scripts/tests/validate_coverage_smoke.py` move in the same commit |

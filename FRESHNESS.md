...
| `INVESTIGATION.md` | plugin author | the question types, the boundary catalog (B1–B14), a completion checklist, the disposition or verdict sets, the closure/counter-search/proportionality rules, the coverage-ledger tables, or the reply shape changes — the pointing surfaces stay pointers; the `investigation:` schema in `CONFIG.md` and its reader `scripts/afk-config.py` move in the same commit when a boundary class is added or removed |

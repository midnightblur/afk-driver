...
| `LANGUAGE.md` | plugin author | the writing doctrine (language rules, ubiquitous-language rules, concision bar incl. its steering-notes section, scope/exceptions) changes — the pointing files stay pointers; specialized bars (`PRD-TEMPLATE.md`, `REPORTING.md`) move in step; a §1 sentence-rule change, or a change to §2's coinage/gloss rules, also updates the synchronized install block in `skills/afk/setup/PLAIN-LANGUAGE.md` same-commit; a skill or agent is **added** → its file opens with the pointer line (`hooks/skill-registry-gate.sh` check D refuses the commit turn otherwise) |
| `LAVISH.md` | plugin author | the pin bumps; a render point (RP row) is added/removed; a playbook id changes; the fallback/forbid/opt-out rules change — update the woven surface(s) in step (skills; RP-10's weave is the install block `skills/afk/setup/LAVISH-SESSIONS.md`); the tooltip-dictionary, tab-title, or page-anatomy contract (paths, matching, injection, backfill, `data-afk-*` grammar) changes — `hooks/lavish-tips.sh` + `hooks/lavish-tips.json` + `hooks/README.md` move in step; the set of `lavish-axi` subcommands that inject changes — the two hooks parse it with identical blocks, so `hooks/lavish-tips.sh` **and** `hooks/lavish-dark.sh` move together, never one alone — and the follow-a-direct-write-with-a-poll instruction rests on which commands inject, so `LAVISH.md` "The page runtime lives in the file" and `LAVISH-KIT.md`'s render note move with them |
| `LAVISH-KIT.md` + `scripts/lavish_render.py` (+ `scripts/lavish/`, `scripts/tests/test_lavish_render.py`) | plugin author | the round-document schema, a component's fields, the skeleton, the two failure modes, the `data-afk-input` contract, the silence rule, or the round response grammar changes — the contract file, the renderer, and its tests move in the same commit (one home, one emitter, one test set); the `data-afk-*` anatomy the renderer emits changes — `LAVISH.md` "Page anatomy" and `hooks/lavish-tips.sh` move with it; the Production-path rule changes — `LAVISH.md` and every woven kit-path render sentence move in step; **a new kit component or group layout lands — revisit `LAVISH.md`'s RP-map Path column and flip every row the kit now renders** (the column records today's coverage, the rule decides, so an unflipped row understates the destination silently) |
...
| `providers/codex/agents/*.toml` | plugin author | an `agents/*.md` role is added, removed, renamed, or changes model/sandbox needs — keep pointer-only stubs; `/afk:setup` copies them and resolves `{{PLUGIN_ROOT}}` |
...
| `skills/afk/bug/LEDGER-FORMAT.md` + `BUNDLE-FORMAT.md` + `CONFIG.md` | `/afk:bug` | the ledger schema/state machine, the bundle grammar, or the `developer:` key set (K1-K4) changes — update `SKILL.md`'s pointers in step |
| `scripts/create-worktree` + `skills/afk/bug/scripts/publish_bug.py` | `/afk:bug` | the `WORKTREE_PATH=`/`ERROR=` contract or the Jira create/transition/comment/backfill payloads change — update `SKILL.md` §`dispatch`/§`capture` in step |
| `scripts/worktree-provision` + `adapters/build-gate/*/worktree-provision.sh` | `/afk:bug`, `/afk:execute`, `/afk:settle-change` | the JSON payload, the answer fields, the exit-code meanings, or the marker format change — update both `CONTRACT.md` files, `CONFIG.md`'s schema rows and `scripts/tests/worktree-provision-smoke.sh` in the same commit |
...
| `adapters/tracker/payload.py` + `scripts/tests/test_tracker_surface.py` | plugin author | the payload error object or the exit code a tracker surface answers a bad payload with changes — both tracker kinds' `CONTRACT.md` and the suite move in the same commit |
...
| `skills/afk/understand/SKILL.md` + `UNDERSTANDING-FORMAT.md` + `shell-template.html` | `/afk:understand` | the subject families / section model / predicates / quiz rules / **meta-header grammar** change — update the mission-control understanding parser (`skills/afk/mission-control/scripts/mc/sections/understanding.py`) in the same commit (lockstep pair); a journal event token is added/renamed — update `skills/afk/to-subtasks/JOURNAL-FORMAT.md` (writer row `understand`/subject `understanding`) same-commit; the skill dir is added/renamed/removed — update `plugin.json` skills array, `README.md` (§3 chain map, §7 artifact tree, §10 skill reference), `CLAUDE.md` skills catalog + journal writer list, and the marketplace description |
...
| `skills/afk/grill-requirements/ROUND.md` | plugin author | the round dossier changes — an item class, the decide rule's conditions, the evidence grades, the decided-card contract, the dossier section set, the audit controls, the steer or escape rules, or the merge rule: every grill pointing at the file is re-read for a restatement that has now gone stale (`/afk:grill-requirements`, `/afk:grill-solution`, `/afk:grill-verification`), and the renderer's component contracts in `LAVISH-KIT.md` move with a field or grade change; a navigability rule changes — rules 1-3 are mechanical (the shape line, the group sections, the card disclosure), so `LAVISH-KIT.md` + `scripts/lavish/` + its tests move in the same commit |
| `skills/afk/grill-solution/HUMAN-SIGNOFF.md` | `/afk:grill-solution` | the human-locked aspect set, a contract grade, or the signing protocol changes — update the signoff-row grammar in `skills/afk/grill-requirements/GRILL-LOG-FORMAT.md`, the SDD §0 register + the aspects' sections in `skills/afk/to-sdd/SDD-TEMPLATE.md`, and `/afk:to-sdd`'s sign-off gate in the same commit |
| `skills/afk/to-meeting-b/MEETING-PLAN-FORMAT.md` + `scripts/validate_agenda.py` (+ `scripts/tests/test_validate_agenda.py`) | plugin author | the agenda grammar, the segment-card field set, the objection-window rules, the disposition set, or the fallback grammar changes — the format file, the checker and its tests move in the same commit, and every producing skill pointing at the format file is re-read for a restatement that has now gone stale (`/afk:to-meeting-b`, `/afk:to-meeting-d`); the company-meeting minute budget changes — both shipped templates move with it, the checker's tests carry them |
| `skills/afk/to-sdd/ADR-TEMPLATE.md` + `skills/afk/to-prd/ADR-FORMAT.md` | plugin author | the ADR status-line grammar changes — the design template owns it, the requirements format carries the synchronized copy at its own emitting site, and both move in the same commit; a status value is added or retired — every skill that reads a record's status moves with them; the `Audited:`-presence provenance rule changes — `/afk:retro`'s source row moves with it |

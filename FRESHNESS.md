...
| AFK pitch page — source `skills/utils/pitch/afk-pitch.html`, published as a claude.ai artifact (URL one-homed in `skills/utils/pitch/SKILL.md`; owner: mvu) | `/afk:pitch` | the skill set, chain map, gate set, or a limitation/WIP item changes — run `/afk:pitch`: diff from the source's embedded snapshot-commit marker, patch stale facts + footer date + marker, republish to the same URL (owner's account only). Manually invoked — deliberately no automatic trigger |
